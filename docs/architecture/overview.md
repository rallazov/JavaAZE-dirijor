...
- `POST /realms/spin` + `DELETE /realms/{job_id}` + `GET /realms/{job_id}` with the `terraform-digitalocean` adapter available when **`DIGITALOCEAN_TOKEN`**, **`DIRIJOR_DO_SSH_PUBLIC_KEY`**, and a terraform binary are configured (Story 2.2 + Story 9.1, 2026-04-18 / 2026-04-23). **`terraform/modules/private-realm`** provisions **`var.agent_count`** VPC-attached droplets (tagged like the Story 2.3 firewall) rather than only an empty VPC + firewall envelope. Story 2.3 (2026-04-19) adds **`EgressPolicyRealmAdapter`** (pre-validate / pre-provision hook → `egress_policy_denied`) and default-deny public Internet egress in Terraform; optional **`DIRIJOR_ALLOW_PUBLIC_EGRESS`** adds explicit outbound to the Internet.
