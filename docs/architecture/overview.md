...
- `POST /realms/spin` + `DELETE /realms/{job_id}` + `GET /realms/{job_id}` with the `terraform-digitalocean` adapter available when **`DIGITALOCEAN_TOKEN`**, **`DIRIJOR_DO_SSH_PUBLIC_KEY`**, mesh bootstrap + Headscale API env + **`DIRIJOR_AGENT_WRAPPER_IMAGE`**, and a terraform binary are configured (Story 2.2 + Story 9.1 + Story 9.2, 2026-04-18 / 2026-04-23 / 2026-04-24). **`terraform/modules/private-realm`** provisions **`var.agent_count`** VPC-attached droplets with per-droplet cloud-init (templatefile) for Docker + OpenClaw wrapper + `tailscale up` to Headscale, rather than only an empty VPC + firewall envelope. Preauth keys for `user_data` are minted **before** `terraform apply`. Story 2.3 (2026-04-19) adds **`EgressPolicyRealmAdapter`** (pre-validate / pre-provision hook → `egress_policy_denied`) and default-deny public Internet egress in Terraform; optional **`DIRIJOR_ALLOW_PUBLIC_EGRESS`** adds explicit outbound to the Internet; Headscale / Tailscale install paths may need public egress or a private control-plane URL (see module README).
